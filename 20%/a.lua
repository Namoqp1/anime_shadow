	...
	while task.wait() do
		if true then
			if game:GetService("Players").LocalPlayer.PlayerGui.UI.HUD.Trial.Visible == true then
				...
				for i,v in pairs(game:GetService("Players").LocalPlayer.PlayerGui.UI.Frames.Pets.Background.Frame.List:GetChildren()) do
					local enemiesFolder = workspace.Server.Enemies["Leaf Village"]
					local children = enemiesFolder:GetChildren()
					if v.Name ~= "UIGridLayout" then
						if #children > 0 then
							local randomIndex = math.random(1, #children)
							local randomEnemy = children[randomIndex]
							local args = {
								[1] = "General",
								[2] = "Pets",
								[3] = "Attack",
								[4] = v.Name,
								[5] = workspace.Server.Enemies:WaitForChild("Leaf Village"):WaitForChild(randomEnemy.Name)
							}
