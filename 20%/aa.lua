...
spawn(function()
	while task.wait() do
		if true then
			if not player.PlayerGui.UI.HUD.Trial.Visible then
				wait(2.5)
				local args = {
					[1] = "Enemies",
					[2] = "Trial_Easy",
				...
				for _, enemy in pairs(workspace:WaitForChild("Server"):WaitForChild("Trial"):WaitForChild("Enemies"):WaitForChild("Easy"):GetChildren()) do
					if enemy:FindFirstChild("Position") then
				...
				if closestEnemy then
					-- วาร์ปไปยังศัตรูที่ใกล้ที่สุด
					player.Character.HumanoidRootPart.CFrame = closestEnemy.CFrame

					-- ส่งคำสั่งโจมตี
					for _, pet in pairs(player.PlayerGui.UI.Frames.Pets.Background.Frame.List:GetChildren()) do
						if pet.Name ~= "UIGridLayout" then
							local args = {
								[1] = "General",
								[2] = "Pets",
								[3] = "Attack",
							...
							game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("Bridge"):FireServer(unpack(args))
							task.wait(1) -- หน่วงเวลาเพื่อหลีกเลี่ยงการโจมตีซ้ำเร็วเกินไป
						end
					end
				end
			end
