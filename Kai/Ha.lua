...
local processedV = {}  -- สร้าง table เพื่อเก็บ v ที่ถูกใช้งานแล้ว
		...
		if config["Automatically Kaitan"] then
			...
			for i,v in pairs(workspace.Server.Pets:GetChildren()) do
				...
				if string.find(v.Name, searchTerm) then
					...
					if table.find(ezxd, v:GetAttribute("Name")) then

						

						-- ตรวจสอบว่า v ถูกใช้งานไปแล้วหรือยัง

						if not processedV[v] then

							-- ถ้า v ยังไม่ถูกใช้งาน

							processedV[v] = true  -- บันทึกว่า v นี้ถูกใช้งานแล้ว

							

							local childCount = #v:GetChildren()  -- นับจำนวนลูกทั้งหมดของ v โดยไม่สนใจชนิด

							

							print("Filtered Children count:", childCount)
